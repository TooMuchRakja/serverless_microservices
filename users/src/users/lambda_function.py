...
# Prepare DynamoDB client with explicit region
USERS_TABLE = os.getenv('USERS_TABLE', None)
dynamodb = boto3.resource('dynamodb', region_name='eu-central-1')
ddbTable = dynamodb.Table(USERS_TABLE)

def lambda_handler(event, context):
    route_key = f"{event['httpMethod']} {event['resource']}"  # STRING typu http method na podstawie ktorego wybierana jest akcja 

    # Defaultowa odpowiedz jezeli zadna opcja z ponizszych sie nie pojawi, po try mamy metody
    ...
    try:
        # Get a list of all Users, pobieranie wszystkich uzytkownikow 
        ...
        # Read a user by ID, pobieranie konkretnego użytkowika 
        ...
        # Delete a user by ID, usuwanie uzytkownika 
        ...
        # Create a new user, tworznenie nowego uzytkownika 
        if route_key == 'POST /users':
            ...
            if 'userid' not in request_json:
                request_json['userid'] = str(uuid.uuid1())  # tworzenie unikatowego stringa dla uzytkownika 
            # update the database
            ddbTable.put_item(
                Item=request_json
            )
