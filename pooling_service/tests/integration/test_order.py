...
def test_order_status(global_config,capsys):
    ...
    response = get_order_status(
        global_config["orders_api_endpoint_url"], # do zmiany zmienne 
...
def test_order_update_process(global_config,capsys):
    ...
    eb_client = boto3.client('events') # eb client - event bridge client
        ...
        Entries=[
            {
                ...
                'EventBusName': global_config['Pooling_Bus_Name'], # do zmiany 
    ...
    response = get_order_status(
        global_config["orders_api_endpoint_url"],
        global_config["regularUserIdToken"],
        order['data']['orderId'],
    )
